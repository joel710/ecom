...
import { checkoutViaWhatsApp } from '../services/whatsapp';

export default function CartSidebar() {
    const { cart, removeFromCart, toggleCart, total } = useCart();

    const handleCheckout = () => {
        checkoutViaWhatsApp(cart, total);
    };
            ...
            <div className="absolute right-0 top-0 bottom-0 w-full max-w-sm bg-white p-6 shadow-2xl flex flex-col animate-slide-left">
                        ...
                        cart.map((item, index) => (
                            <div key={`${item.id}-${index}`} className="flex items-center space-x-4 animate-fade-in">
                                ...
                                <div className="flex-grow">
                                    ...
                                    <p className="text-sm font-bold text-google-blue">{item.price.toLocaleString('fr-FR')} FCFA</p>
                                </div>
                                <button
                ...
                <div className="border-t pt-6 mt-4">
                    ...
                    <button
                        ...
                        className="w-full bg-[#25D366] text-white py-4 rounded-full font-medium shadow-lg active:scale-95 transition-all hover:bg-[#128C7E] disabled:opacity-50 disabled:cursor-not-allowed flex justify-center items-center gap-2"
                    >
